...
public class GlobalReactiveErrorHandler {
    ...
    @ResponseBody
    public Mono<ApiResponse<String>> handleMethodArgumentNotValid(
            MethodArgumentNotValidException exception) {
        List<String> errors = exception.getBindingResult().getFieldErrors().stream()
                .map(error -> error.getField() + ": " + error.getDefaultMessage())
                .collect(Collectors.toList());

        return Mono.just(new ApiResponse<>(FAIL, HttpStatus.BAD_REQUEST.value(), String.join(",", errors)));
    }

    @ExceptionHandler(UserNotFoundException.class)
    @ResponseStatus(HttpStatus.NOT_FOUND)
    @ResponseBody
    public Mono<ApiResponse<String>> handleUserNotFoundException(UserNotFoundException exception) {
    ...
    @ResponseBody
    public Mono<ApiResponse<String>> handleSqlException(SQLException exception) {
    ...
    @ResponseStatus(HttpStatus.INTERNAL_SERVER_ERROR)
    @ResponseBody
    public Mono<ApiResponse<String>> handlePropertyAccessException(PropertyAccessException exception) {
        return exceptionMessage(exception, HttpStatus.INTERNAL_SERVER_ERROR.value(), INTERNAL_ERROR);
    }
    ...
    @ResponseStatus(HttpStatus.NOT_FOUND)
    @ResponseBody
    public Mono<ApiResponse<String>> handleRecordNotFounException(RecordNotFounException exception) {
    ...
    @ResponseStatus(HttpStatus.BAD_REQUEST)
    @ResponseBody
    public Mono<ApiResponse<String>> handleBadRequestExceptions(BadRequestException exception) {
    ...
    @ResponseBody
    public Mono<ApiResponse<String>> handleIllegalArgumentException(IllegalArgumentException exception) {
    ...
    @ResponseStatus(HttpStatus.BAD_REQUEST)
    @ResponseBody
    public Mono<ApiResponse<String>> handleMethodArgumentTypeMismatch(
    ...
    @ResponseStatus(HttpStatus.INTERNAL_SERVER_ERROR)
    @ResponseBody
    public Mono<ApiResponse<String>> handlerGlobalError(Exception exception) {
        return exceptionMessage(exception, HttpStatus.INTERNAL_SERVER_ERROR.value(), INTERNAL_ERROR);
    }

    private static Mono<ApiResponse<String>> exceptionMessage(Exception exception, int statusCode, String errorMessage) {
        log.warn("{} {} occurred", Exception.class.getSimpleName().toUpperCase(), exception.getMessage());
        return Mono.just(new ApiResponse<>(FAIL, statusCode, errorMessage));
    }
}
